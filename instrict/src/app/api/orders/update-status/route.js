  ...
  try {
    ...
    if (updateError) {
      ...
      Sentry.captureException(updateError, {
        tags: { flow: 'order-status-update', portal: 'vendor' },
    ...
    if (newStatus === 'ready' && order.fulfillment_type === 'delivery') {
      notifyRidersOfNewJob(supabase, {
        ...
        Sentry.captureException(err, {
          tags: { flow: 'order-status-update', step: 'notify-riders' },
          extra: { orderId },
        });
      });
    }
    ...
    console.error('order status update route error:', err);
    Sentry.captureException(err, {
      tags: { flow: 'order-status-update', step: 'unhandled' },
    });
