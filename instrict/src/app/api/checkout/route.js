...
export async function POST(req) {
  try {
    ...
    if (!Number.isFinite(subtotal) || subtotal <= 0) {
      ...
      Sentry.captureMessage('Invalid subtotal computed at checkout', {
        level: 'warning',
        tags: { flow: 'checkout' },
        extra: { isPrintOrder, bodySubtotal: body.subtotal, items },
      });
    ...
    try {
      ...
      Sentry.captureException(err, {
        tags: { flow: 'checkout', step: 'paystack-initialize' },
        extra: { reference, vendorId, total, isPrintOrder },
      });
    ...
    Sentry.captureException(err, {
      tags: { flow: 'checkout', step: 'unhandled' },
    });
