...
export async function POST(req) {
  try {
    ...
    if (!Number.isFinite(reward) || reward <= 0) {
      Sentry.captureMessage('Invalid errand reward amount at checkout', {
        level: 'warning',
        tags: { flow: 'errand-checkout' },
        extra: { errandId, reward: errand.reward },
      });
    ...
    try {
      ...
      Sentry.captureException(err, {
        tags: { flow: 'errand-checkout', step: 'paystack-initialize' },
        extra: { reference, errandId, total },
      });
      return NextResponse.json({ error: 'Could not start payment' }, { status: 500 });
    }
  } catch (err) {
    console.error('errand checkout route error:', err);
    Sentry.captureException(err, {
      tags: { flow: 'errand-checkout', step: 'unhandled' },
    });
