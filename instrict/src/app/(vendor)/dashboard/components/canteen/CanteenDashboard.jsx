...
import { LayoutDashboard, UtensilsCrossed, ClipboardList, Wallet, Settings, Bell, Ban } from 'lucide-react';
...
// Store open/closed status is only relevant on sections where the vendor
// is actively managing live orders. Wallet, Settings, Community, and
// Notifications have nothing to do with whether the store is open right
// now, so the banner is noise there.
const STORE_BANNER_SECTIONS = ['overview', 'kitchen', 'menu'];

export default function CanteenDashboard({ vendor, activeSection, onSectionChange, onVendorUpdate, isSuspended }) {
  ...
  // can scroll to and ring-highlight that specific post. Not cleared on
  // every section change — it just sits unused until the vendor opens
  // Community again via a notification.
  ...
  const bottomNav = [
    ...
    { label: 'Kitchen', section: 'kitchen',         icon: UtensilsCrossed },
    { label: 'Menu',    section: 'menu',            icon: ClipboardList },
  ...
  const renderSection = () => {
    switch (activeSection) {
      case 'overview':
        return <CanteenOverview vendorUserId={vendor.user_id} />;
      case 'kitchen':
        return <LiveKitchen vendorUserId={vendor.user_id} isSuspended={isSuspended} />;
      case 'menu':
        return <MenuManager vendorUserId={vendor.user_id} isSuspended={isSuspended} />;
      case 'wallet':
        return <VendorWallet vendor={vendor} isSuspended={isSuspended} />;
      case 'notifications':
        return <VendorNotifications vendor={vendor} onNavigate={handleNotificationNavigate} />;
      case 'community':
        return <CommunityFeed authorType="vendor" isSuspended={isSuspended} highlightPostId={highlightPostId} />;
      case 'settings':
        return <VendorSettings vendor={vendor} onUpdate={onVendorUpdate} />;
      case 'help':
        return <HelpDesk authorType="vendor" vendor={vendor} />;
      default:
        return (
          ...
          <div className="flex items-start gap-2.5 rounded-2xl border border-rose-500/20 bg-rose-500/5 px-4 py-3.5 mb-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                ...
                {' '}You can still view existing orders, but new orders, menu changes, and community posts are locked until this is resolved.
              </p>
            </div>
          </div>
        )}
