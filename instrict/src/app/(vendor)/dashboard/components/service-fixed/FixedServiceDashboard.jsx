...
import { LayoutDashboard, ClipboardList, Tags, Image as ImageIcon, Wallet, Bell, Ban } from 'lucide-react';
...
const STORE_BANNER_SECTIONS = ['overview', 'orders', 'pricing', 'portfolio'];

export default function FixedServiceDashboard({ vendor, activeSection, onSectionChange, onVendorUpdate, isSuspended }) {
  ...
  // bottom nav / bell icon already use. This dashboard's "order" section is
  // named 'orders' (not 'kitchen' like Canteen's), hence the sectionMap
  // passed to VendorNotifications below.
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('unread-count-fixed-service')
      .on('postgres_changes', {
  ...
  const bottomNav = [
    ...
    { label: 'Pricing',   section: 'pricing',        icon: Tags },
  ...
  const renderSection = () => {
    switch (activeSection) {
      case 'overview':      return <FixedServiceOverview vendorUserId={vendor.user_id} />;
      case 'orders':        return <IncomingOrders vendorUserId={vendor.user_id} isSuspended={isSuspended} />;
      case 'pricing':       return <PriceMatrix vendorUserId={vendor.user_id} isSuspended={isSuspended} />;
          ...
          <div className="flex items-start gap-2.5 rounded-2xl border border-rose-500/20 bg-rose-500/5 px-4 py-3.5 mb-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                ...
                {' '}You can still view existing orders, but new orders, pricing/portfolio changes, and community posts are locked until this is resolved.
              </p>
            </div>
          </div>
        )}
